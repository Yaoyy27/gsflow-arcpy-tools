#--------------------------------
# Name:         gsflow_prism_400m_normals.py
    ...
    try:
        ...
        for data_name in data_name_list:
            ...
            prism_normal_re = re.compile(
                '^(west)_(%s)_\d{2}_mean.img.*$' % '|'.join(data_name_list),
                re.IGNORECASE)
            ...
            for month in month_list:
                ...
                input_name = 'west_{0}_{1}.img'.format(data_name.lower(), month)
                output_name = 'west_{0}_{1}_mean.img'.format(
